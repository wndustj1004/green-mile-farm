...
- 챌린지 기간의 **정답은 DB `settings.challenge_end`**(관리자 `/admin/settings`에서 수정).
  `supabase/schema.sql`의 기본값 `2026-07-26`은 초기 스키마 값일 뿐이니 근거로 삼지 말 것.
- **참여 인원·누적 감축량·수확자 수는 이 문서에 적지 않는다.**
  설정값이 아니라 **회원가입·인증 승인에 따라 실시간 집계되는 값**(`challenge_stats()` RPC, §5-6)이므로
  항상 화면(메인 '실시간 임팩트' / `/admin` 통계)에서 확인할 것.
