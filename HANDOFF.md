# 그린마일 팜 — 새 세션 인수인계 (2026-08-21 갱신)
...
운영: 참여 15명 규모, 목표 감축 **5kg**, 교통 4수단(걷기·자전거·버스·지하철). 챌린지 **2026-08-20(목) 마감 — 종료됨**(settings.challenge_end).
...
## 3. 현재 상태 (★중요)
- **운영 배포 완료.** `main` = `4f9a0b7` (origin/main과 동기화). 로컬=원격=운영 일치.
...
- **14는 2026-08-11 적용 완료** (계정 복구 — 아이디 찾기/비밀번호 재설정). 표 2개 생성 + anon 차단 확인함.
  코드도 같은 날 배포됨(`9eff1b4`). 아래 5번 "계정 복구" 참고.
  ※ 비밀번호 실제 변경 경로(`updateUserById`)도 2026-08-11 관리자 계정으로 검증 완료
    (`scripts/reset-password.mjs` 실행 → 새 비밀번호로 로그인 성공 확인).
    화면 2단계도 같은 함수를 호출하므로 동일하게 동작함.
- **15는 2026-08-21 적용 완료** (챌린지 마감 후 마지막 주 보너스 즉시 확정).
  마지막 주(8/17~8/23) 3건 +1.820kg 확정 — `scripts/check-bonus.mjs` 전원 ✅ 검산 통과.
...
- **SQL은 코드 배포와 별개**: 새 마이그레이션 파일 만들면 사용자가 Supabase SQL Editor에서 **직접 실행**해야 함. `supabase/NN_*.sql`. (02~15 이미 실행됨)
...
### 계정 복구 — 아이디 찾기 / 비밀번호 재설정 `app/login/find/` (2026-08-11 추가)
참가자가 아이디·비밀번호를 잊었을 때 **회원가입 때 적은 정보로 본인 확인**해서 되찾는 기능. `/login`에 "아이디 · 비밀번호를 잊으셨나요?" 링크.
- **왜 이메일 링크 방식이 아닌가**: 참여 15명 규모라 메일 발송 인프라(SMTP)가 없고 Supabase 기본 메일은 시간당 발송 수가 매우 적어 실패 위험이 큼. → 가입정보 대조 방식 채택.
- **아이디 찾기**: 이름 + 휴대폰 + 이메일 3개가 모두 맞으면 **아이디 전체 표시**(마스킹 안 함) + 가입일.
- **비밀번호 재설정**: 2단계. ① 아이디 + 이름 + 휴대폰 + 이메일 **4개 전부** 일치 → **10분짜리 1회용 열쇠(token)** 발급 → ② 새 비밀번호 입력. 비밀번호는 해시로 저장돼 **원본을 알려줄 수 없음** → 재설정만 가능(화면에도 안내문 있음).
- **안전장치 4가지**: ① 재설정은 4개 항목 전부 일치 ② 10분 안에 5회 실패 시 차단(IP·아이디/이메일 각각 집계) ③ 열쇠는 **원본을 저장하지 않고 SHA-256 지문만** DB에 보관, 10분 만료·1회용 ④ **운영진(`is_admin`) 계정은 이 경로로 재설정 불가**(탈취 시 피해가 커서) → `scripts/reset-password.mjs` 사용.
- **틀렸을 때 항상 같은 문구**(`NO_MATCH_MSG`) — 어느 항목이 틀렸는지 알려주면 남의 정보를 캐낼 수 있어서 구분하지 않음.
- 입력값 정리 규칙은 `lib/recovery.ts` 한 곳: 휴대폰=숫자만 비교(하이픈 유무 무관), 이메일=소문자, 이름=공백 제거. 임계값(10분·5회)도 여기 `RATE_LIMIT`.
- SQL = **`supabase/14_account_recovery.sql`** — 표 2개(`recovery_attempts`, `password_reset_tokens`), 둘 다 **RLS 정책 0개 + anon/authenticated 권한 회수** = 서버(service_role)만 접근.
- 운영진 도구: `node --env-file=.env.local scripts/reset-password.mjs <아이디> <새비밀번호>` — 본인확인을 못 하는 참가자나 운영진 본인 계정용.
...
### 관리자 참가자 명부 `app/admin/participants/page.tsx`
전원 목록 + 1인당 집계 + CSV 다운로드 링크. 열 = 이름·아이디·연락처·인증·거리·**순감축량**·**감축량**·수확.
- ★ **감축량 열 2개의 차이**(2026-08-11 추가):
  - **순감축량**(`kg`) = 승인 인증분만 합산. (예전 `감축` 열 — 이름만 바뀌고 값은 동일)
  - **감축량**(`totalKg`) = 순감축량 + **확정(settled)된 이벤트 적립**. `admin_bonus_awards()` RPC 1번으로 전원분을 받아 합산하며, **대시보드의 `effective_reduction_g`와 값이 일치**(47명 전원 대조 확인). 이벤트가 붙은 사람은 옆에 `(+2.99)` 회색 표시.
  - **보상 인원(상위 15명) 선정 기준 = `감축량` 열** (2026-08-11 사용자 결정).
  - 진행 중인 주의 `settled=false` "적립 예정"은 **포함하지 않음** — 대시보드·`/my`와 동일 규칙.
- **엑셀식 정렬**: `인증`·`거리`·`순감축량`·`감축량` **열 제목을 누르면 정렬**. 처음 누르면 내림차순(▼), 다시 누르면 오름차순(▲). 정렬 중인 열은 초록 굵게 + 숫자도 강조.
- 상태는 **URL 쿼리**로만 관리(`?sort=count|dist|kg|totalKg&dir=desc|asc`) — 서버 컴포넌트라 클라이언트 JS 없음. `buildHref()`가 필터(`harvested`)와 정렬을 합쳐 주소를 만들어, **한쪽을 바꿔도 다른 쪽이 유지**됨.
- 값이 같으면 **가입순이 그대로 유지**됨(JS sort는 안정 정렬). 0kg 참가자들이 뒤섞이지 않는 이유.
- ※ **`수확 🍅` 판정은 계속 순감축량 기준**. 보너스 산정이 수확 여부를 참조하므로(12·13 SQL의 `totals`), 여기에 보너스를 넣으면 순환 참조가 됨. 의도된 것이니 바꾸지 말 것.
- ※ 수확 달성자는 **주간 랭킹 보너스 대상에서 제외**되므로 이벤트 적립이 `steady3` 1kg뿐임(이○○ 18회 인증인데 +1.00인 이유). 보상 순위에 영향 있으니 인지할 것.
...
- **확정(settled) vs 예정**: 주가 끝나야(월요일 0시 KST) 순위가 확정 → 진행 중인 주는 `settled=false`로 "적립 예정" 표시만 하고 누적 감축·작물 성장엔 **미반영**.
  **단, 챌린지 마감일이 지나면(오늘 KST > `challenge_end`) 남은 주도 즉시 확정**(15번 SQL) — 마감 후엔 인증이 더 들어올 수 없어 순위가 이미 결정되기 때문.
...
|---|---|---|---|---|---|
| `weekly_rank` | 주간 랭킹 보너스 🏆 | 한 주(월~일) 감축량 상위 3명. **수확 도달자 제외** | 그 주 감축량의 50% | 그 주가 끝난 월요일 0시(KST). **또는 챌린지 마감일이 지나면 즉시**(15) | 12·15 |
...
- **2026-08-21 마감 정산(15)**: 챌린지가 주 중간(8/20 목)에 끝나 마지막 주(8/17~8/23)가 계속 "적립 예정"으로 남던 문제를 해결. `settled` 조건에 **"오늘 KST > challenge_end"** 를 OR로 추가 → 조○○ +1.076 / 최○○ +0.434 / 최△△ +0.310 = **3명 +1.820kg** 확정. **수확 달성자 변동 없음.** 마감일을 뒤로 미루면 다시 "적립 예정"으로 돌아감(의도된 동작).
