...
## 3. 현재 상태 (★중요)
- **운영 배포 완료.** `main` = `9eff1b4` (origin/main과 동기화). 로컬=원격=운영 일치.
...
전원 목록 + 1인당 집계(인증 횟수·거리·감축량·수확 여부) + CSV 다운로드 링크.
- **엑셀식 정렬**(2026-08-11 추가): `인증`·`거리`·`감축` **열 제목을 누르면 정렬**. 처음 누르면 내림차순(▼), 다시 누르면 오름차순(▲). 정렬 중인 열은 초록 굵게 + 숫자도 강조.
- 상태는 **URL 쿼리**로만 관리(`?sort=count|dist|kg&dir=desc|asc`) — 서버 컴포넌트라 클라이언트 JS 없음. `buildHref()`가 필터(`harvested`)와 정렬을 합쳐 주소를 만들어, **한쪽을 바꿔도 다른 쪽이 유지**됨.
- 값이 같으면 **가입순이 그대로 유지**됨(JS sort는 안정 정렬). 0kg 참가자들이 뒤섞이지 않는 이유.
- ※ 이 페이지의 집계는 **인증분만** 합산 — 보너스는 포함하지 않음(대시보드·`/my`와 숫자가 다를 수 있음).
